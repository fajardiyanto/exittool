...
func vipsGetAllMetadataKey(image *C.VipsImage) (res []string) {
	ss := C.vips_image_get_fields(image)
	defer C.free(unsafe.Pointer(ss))
	ln := C.vips_get_all_len_metadata_key(ss)
	for _, s := range unsafe.Slice(ss, ln) {
		if s != nil {
