...
*/
func decodeNRGBA(startOffset uint32, msgLen uint32, rgbImage *image.NRGBA) (ori, message []byte) {
	...
	for x := 0; x < width; x++ {
		for y := 0; y < height; y++ {
			...
			if bitIndex > 7 {
				// when we have filled up a byte, move on to the next byte
				bitIndex = 0
				byteIndex++

				if byteIndex >= msgLen+startOffset {
					if startOffset < msgLen+startOffset {
						return ori, message[startOffset : msgLen+startOffset]
					} else {
						return ori, nil
					}
				}
...
func decode(startOffset uint32, msgLen uint32, pictureInputFile image.Image) (message []byte) {
	...
	rgbImage := imageToNRGBA(pictureInputFile)
	_, msg := decodeNRGBA(startOffset, msgLen, rgbImage)
	return msg
}
...
func GetMessageSizeFromImage(pictureInputFile image.Image) (size uint32) {
	...
	return combineToInt(sizeAsByteArray[0], sizeAsByteArray[1], sizeAsByteArray[2], sizeAsByteArray[3])
}
